...
constexpr float NEW_CHEMICAL_BIAS = 10000.0f;  // Bias for new chemicals
...
void update(std::vector<int> &counts, std::vector<Rule> &rules, int &n_rules, 
    ...
    // Supply: convert all empty to chemical 1
    int to_convert = counts[0];
    counts[0] = 0;
    counts[1] += to_convert;
    ...
    if (matching_rule == -1 && n_rules < MAX_RULES) {
        ...
        // Enumerate all valid product combinations with weights
            ...
            for (int p2 = p1; p2 <= N_CHEMICALS && p1 + p2 <= reactant_mass; ++p2) {
                for (int p3 = p2; p3 <= N_CHEMICALS; ++p3) {
                    if (p1 + p2 + p3 <= reactant_mass) {
                        ...
                        // Calculate weight: inversely proportional to product concentrations
                        // Zero concentration products get NEW_CHEMICAL_BIAS, others get 1/concentration
                        double weight = 1.0;
                        for (int prod : {p1, p2, p3}) {
                            if (counts[prod] == 0) {
                                weight *= NEW_CHEMICAL_BIAS;
                            } else {
                                weight *= (1.0 / counts[prod]);
                            }
                        }
        ...
        if (!valid_products.empty()) {
            ...
            // Randomly select one valid product combination with weighted probability
            std::uniform_real_distribution<> weight_dis(0.0, cumulative_weights.back());
            double rand_weight = weight_dis(gen);
            size_t selected_idx = 0;
            for (size_t i = 0; i < cumulative_weights.size(); ++i) {
                if (rand_weight <= cumulative_weights[i]) {
                    selected_idx = i;
                    break;
                }
            }
            
            std::array<int, 3> selected = valid_products[selected_idx];
...
int main() {
    ...
    dir_name << "N_" << TOTAL_MOLECULES << "_b_" << NEW_CHEMICAL_BIAS;
    std::filesystem::path output_dir = std::filesystem::current_path() / "outputs" / "evolveProductsTimeseries" / dir_name.str();
