...
def main_evolve_products(N, b, subdir):
    # Get script directory
    script_dir = Path(__file__).parent
    
    # Load data from outputs in script directory
    data_dir = script_dir / "outputs" / subdir / f"N_{N}_b_{b}"
    counts_file = data_dir / "counts.tsv"
    ...
    ax.set_ylabel('Molecule Count', fontsize=14)
    ax.set_yscale('log')
    ax.set_title(f'Chemical Populations Over Time (N={N}, b={b})', fontsize=16)
    ax.grid(True, alpha=0.3)
    
    # Save figure to plots in script directory
    output_dir = script_dir / "plots" / subdir
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"populations_N_{N}_b_{b}.png"
    plt.tight_layout()
    plt.savefig(output_file, dpi=300)
    print(f"Plot saved to {output_file}")
    plt.close()
...
if __name__ == "__main__":
    ...
    b = 10000
    subdir = "evolveProductsTimeseries"
    main_evolve_products(N, b, subdir)