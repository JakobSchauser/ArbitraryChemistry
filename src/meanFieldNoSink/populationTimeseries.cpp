...
void update(std::vector<int> &counts, std::vector<Rule> &rules, int &n_rules, 
    ...
    for (int c = 0; c <= N_CHEMICALS; ++c) {
        total_weighted += (long long)(counts[c] * volatility[c] * 1000);
    }
    
    if (total_weighted == 0) return;
    
    std::uniform_real_distribution<> sample_dis(0.0, (double)total_weighted);
    int reactants[3];
    for (int i = 0; i < 3; ++i) {
        long long rand_val = (long long)sample_dis(gen);
        long long cumsum = 0;
        for (int c = 0; c <= N_CHEMICALS; ++c) {
    ...
    if (matching_rule == -1 && n_rules < MAX_RULES) {
        ...
        // Enumerate all valid product combinations
